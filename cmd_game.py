...
def check_chests(player, last_n):
    for card_i in range(last_n):
        if sum(1 for card in bot.hand if
               Card.value[card.value] == Card.value[player.hand[-1 * (card_i + 1)].value]) == 4:
            bot.chest_number += 1
            bot.hand = [card for card in player.hand if
                        not Card.value[card.value] == Card.value[player.hand[-1 * (card_i + 1)]]]
            card_i -= 1
...
def player_turn():
    while (1):
        ...
        if line == "hand":
            ...
            print(f"{bot.name}: I don't understand you. Try again")
                ...
                while (1):
                    ...
                    if answer == "yes" and correct_answer == 1:
                        ...
                        if sum(1 for card in bot.hand if Card.value[card.value] == Card.value[bot.hand[-1].value]) == 4:
                            bot.chest_number += 1
                            bot.hand = [card for card in bot.hand if
                                        not Card.value[card.value] == Card.value[bot.hand[-1]]]
                            if player.chest_number + bot.chest_number == 13:
                                end_game()
                        return
                    elif answer == "no" and correct_answer == 0:
                        print(f"{bot.name}: Okay. Your turn.")
                        draw_cards(card_deck, bot, 1)
                        if sum(1 for card in player.hand if
                               Card.value[card.value] == Card.value[player.hand[-1].value]) == 4:
                            player.chest_number += 1
                            player.hand = [card for card in player.hand if
                                           not Card.value[card.value] == Card.value[player.hand[-1]]]
                            if player.chest_number + bot.chest_number == 13:
                                end_game()
                        return
                    else:
